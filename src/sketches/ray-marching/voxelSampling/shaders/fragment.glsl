...
vec2 sample_lpv_trilin(vec3 p){
    p=p*size;
    float inr=inrange(p,vec3(0.),size);
    vec3 pc=clamp(p,vec3(0.),size);
    ...
    return vec2(tril.x,(1.-inr)*cubedist);
...
#ifndef M_DIVPI
#define M_DIVPI.3183098861837907
#endif
...
vec4 sh_irradiance_probe(vec4 v){
    ...
    const float sh_c1=SHSharpness*2./3.;
...
void doCamera(out vec3 camPos,out vec3 camTar,in float time,in vec4 m){
    if(max(m.z,m.w)<=0.){
        float an=1.5+sin(time*.1)*3.;
        camPos=vec3(6.5*sin(an),1.,6.5*cos(an));
        camTar=vec3(.5,.5,.5);
    }else{
        float an=10.*m.x-5.;
        camPos=vec3(6.5*sin(an),10.*m.y-5.,6.5*cos(an));
        camTar=vec3(.5,.5,.5);
    }
}
...
vec3 doLighting(in vec3 pos,in vec3 nor,in vec3 rd,in float dis,in vec4 mal)
{
    ...
    col*=10.*vec3(shade_probe(shr,shn));
...
{
    const float maxd=10.;// max trace distance
    const float precis=1.;// precision of the intersection
    float h=precis*2.;
    float t=0.;
    float res=-1.;
...
vec3 calcNormal(in vec3 pos)
{
    ...
    const float eps=.01;// precision of the normal computation
...
void mainImage(out vec4 fragColor,in vec2 fragCoord)
{
    ...
    for(int i=0;i<samps;i++){
        ...
        vec3 rd=normalize(camMat*vec3(p.xy+dxy,2.));// 2.0 is the lens length
    ...
    for(int i=0;i<samps;i++){
        vec3 x=cols[i];
        float w=exp(-length(x-colavg2)/.2);
        col+=w*x;
        sum2+=w;
    }

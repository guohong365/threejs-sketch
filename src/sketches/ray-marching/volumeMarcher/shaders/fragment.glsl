...
vec2 mapTexture(in vec3 pos)
{
    vec2 res=vec2(1e10,0.);
    
    {
        vec3 d1p=pos;
        float d1=sdTexture(d1p);
...
vec2 raycastTexture(in vec3 ro,in vec3 rd){
    vec2 res=vec2(-1.,-1.);
    float t=0.;
    for(int i=0;i<64;i++)
    {
...
vec3 calcNormalTexture(vec3 p){
    return texture(iChannel0Cube,p).rgb;
}

vec3 renderTexture(in vec3 ro,in vec3 rd){
    vec3 col=vec3(10.,10.,10.)/255.;
    
    vec2 res=raycastTexture(ro,rd);
    float t=res.x;
    float m=res.y;
    ...
    if(m>-.5){
        ...
        vec3 nor=(m<1.5)?vec3(0.,1.,0.):calcNormalTexture(pos);
...
vec3 getSceneColorTexture(vec2 fragCoord){
...
void mainImage(out vec4 fragColor,in vec2 fragCoord){
    ...
    for(float aaY=0.;aaY<AA_size;aaY++)
    {
        ...
        {
            tot+=getSceneColorTexture(fragCoord+vec2(aaX,aaY)/AA_size);
            count+=1.;
        }
    }
    tot/=count;
